...
class UserInfo {
    ...
    async task_true() { // 查询完成任务 && 未领取的任务
        ...
        await this.task_list() //refresh taskList
        for (let i in this.taskList) {
            let isComplete = this.taskList[i].isComplete //finish status 当为true时表示任务完成 反之则未完成
            let taskId = this.taskList[i].taskId
            let taskName = this.taskList[i].taskName
            let isReceiveReward = this.taskList[i].isReceiveReward //领取状态
            let classify = this.taskList[i].classify
            if (taskId !== "multi_times" && isComplete == true && isReceiveReward == false) {
                console.log("任务" + taskName + "完成未领取");
                await this.task_receive(2, classify, taskId)//领取奖励
            } else {
                return console.log("目前发现没有可以领取的任务奖励呢!");
            }
        }
    }
    ...
    async task_receive(classify, taskId) { // 领取水滴任务列表的水滴
        let options = {
            method: 'POST',

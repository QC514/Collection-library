/**
 * 域名youhui.95516.com，抓Authorization的值，不要带上Bearer，要它后面的值
 * 变量
        ...
        axios.request(options).then(async function (response) {
            try {
                ...
                if (response.status == 200) {
                    if (signedIn) {
                        days = response.data.signInDays.days;
                        currentDay = response.data.signInDays.current.days;
                        log("今天是第" + currentDay + "天签到,今日已签到成功,目前已连续签到" + days + "天");
                    }
                } else {
                ...
                log(`异常：${response.statusText}，原因：${response.statusText}`)
            }
